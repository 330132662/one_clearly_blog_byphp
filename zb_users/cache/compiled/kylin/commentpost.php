...
<a rel="nofollow" id="cancel-reply" href="#divCommentPost" style="display:none;color:#00b8e8">
  ...
  <p class="comment-notes">◎欢迎参与讨论，请在这里发表您的看法、交流您的观点。</p>
	<form id="frmSumbit" class="comment-form" target="_self" method="post" action="<?php  echo $article->CommentPostUrl;  ?>" >
...
<?php }else{  ?>
	<p class="comment-form-author"><label for="inpName">名称<span class="required">*</span></label><input type="text" name="inpName" id="inpName" class="text" value="<?php  echo $user->Name;  ?>" size="30" tabindex="1" maxlength="100"/></p>
	<p class="comment-form-email"><label for="inpEmail">邮箱</label><input type="text" name="inpEmail" id="inpEmail" class="text" value="<?php  echo $user->Email;  ?>" size="30" tabindex="2" maxlength="100"/></p>
	<!--<p class="comment-form-url"><label for="inpHomePage">网址</label><input type="text" name="inpHomePage" id="inpHomePage" class="comment-form-url text" value="<?php  echo $user->HomePage;  ?>" size="30" tabindex="3" maxlength="200"/> </p>-->
...
<?php } ?>
	...
	<p><textarea name="txaArticle" id="txaArticle" class="text" cols="50" rows="4" tabindex="5" placeholder="您好，<?php if ($user->ID>0) { ?><?php  echo $user->StaticName;  ?><?php } ?>请发表您的评论!"></textarea></p>
